...
public class Ticket extends Element implements Serializable, ConvertibleToCSV {
    ...
    private Integer price; // может быть null, > 0
    private Double discount; // не null, > 0, <= 100
    ...
    public Ticket(long id, String name, Coordinates coordinates, Integer price,
                Double discount, Boolean refundable, TicketType type, Venue venue, int owner_id) {
        this(id, name, coordinates, LocalDate.now(), price, discount, refundable, type, venue, owner_id);
    }
     ...
     */
    public Ticket() {
        this(1, "Default Ticket", new Coordinates(0, 0), 100, 10.0, true, TicketType.VIP, 
    ...
    public void setRefundable(boolean ref) {
        this.refundable = ref;
    }
...
public Ticket clone() {
    ...
    copy.setCoordinates(new Coordinates(this.coordinates.getX(), this.coordinates.getY()));
    copy.setCreationDate(this.creationDate);
    copy.setPrice(this.price);
    copy.setType(this.type);
    copy.setVenue(this.venue != null
        ? new Venue(
            this.venue.getId(),
            this.venue.getName(),
